...
use WP4Odoo\Modules\MC4WP_Module;
use WP4Odoo\Modules\MC4WP_Handler;
use WP4Odoo\Logger;
use PHPUnit\Framework\TestCase;

/**
 * Unit tests for MC4WP_Module, MC4WP_Handler, and MC4WP_Hooks.
 *
 * Tests module configuration, handler data loading/saving, parse methods,
 * and hook guard logic.
 */
class MC4WPModuleTest extends TestCase {

	private MC4WP_Module $module;
	...
	protected function setUp(): void {
		...
		$this->module  = new MC4WP_Module( wp4odoo_test_client_provider(), wp4odoo_test_entity_map(), wp4odoo_test_settings() );
		$this->handler = new MC4WP_Handler( new Logger( 'mc4wp', wp4odoo_test_settings() ) );
	}

	// ─── Module Identity ───────────────────────────────────

	public function test_module_id_is_mc4wp(): void {
		$this->assertSame( 'mc4wp', $this->module->get_id() );
	}

	public function test_module_name(): void {
		$this->assertSame( 'Mailchimp for WP', $this->module->get_name() );
	}
	...
	public function test_dependency_available_with_mc4wp(): void {
		$status = $this->module->get_dependency_status();
		$this->assertTrue( $status['available'] );
	}

	public function test_dependency_has_no_warning_for_valid_version(): void {
		// MC4WP_VERSION is 4.9.0, TESTED_UP_TO is 4.10.
		// 4.9.0 is within the 4.10 range, so no warning.
		$status = $this->module->get_dependency_status();
	...
	public function test_load_subscriber_returns_data_for_valid_subscriber(): void {
		...
		$GLOBALS['_wp_user_meta'][1]['mc4wp_subscribed_to'] = [ 'abc123', 'def456' ];
		...
		$this->assertSame( [ 'abc123', 'def456' ], $data['list_ids'] );
	}
	...
	public function test_save_subscriber_creates_new(): void {
		...
		$this->assertSame( [ 'abc123' ], $GLOBALS['_wp_user_meta'][ $id ]['mc4wp_subscribed_to'] ?? [] );
	}
	...
	public function test_save_list_creates_new(): void {
		...
		$this->assertGreaterThan( 0, $id );
		$this->assertNotEmpty( $GLOBALS['_mc4wp_lists'] );
	}

	public function test_save_list_updates_existing(): void {
		$GLOBALS['_mc4wp_lists'][8] = [
			'id'          => 8,
			'name'        => 'Existing List',
	...
	public function test_on_subscriber_created_enqueues_create(): void {
		$GLOBALS['_wp_options']['wp4odoo_module_mc4wp_settings'] = [ 'sync_subscribers' => true ];

		// Set up a user so email resolves.
		$user              = new \WP_User( 1 );
		$user->user_email  = 'test@example.com';
		$GLOBALS['_wp_users'][1] = $user;

		$form = new \MC4WP_Form();
		$this->module->on_subscriber_created( $form, [ 'EMAIL' => 'test@example.com' ] );

		$this->assertQueueContains( 'mc4wp', 'subscriber', 'create', 1 );
	}

	public function test_on_subscriber_created_skips_when_disabled(): void {
		$GLOBALS['_wp_options']['wp4odoo_module_mc4wp_settings'] = [ 'sync_subscribers' => false ];
	...
	public function test_on_subscriber_created_skips_zero_id(): void {
		...
		// No user registered for this email, so resolve returns 0.
		$form = new \MC4WP_Form();
		$this->module->on_subscriber_created( $form, [ 'EMAIL' => 'nonexistent@example.com' ] );
	...
	public function test_pull_subscriber_skipped_when_disabled(): void {
		$GLOBALS['_wp_options']['wp4odoo_module_mc4wp_settings'] = [ 'pull_subscribers' => false ];
	...
	public function test_pull_list_skipped_when_disabled(): void {
		$GLOBALS['_wp_options']['wp4odoo_module_mc4wp_settings'] = [ 'pull_lists' => false ];
